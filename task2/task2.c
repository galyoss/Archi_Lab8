...
void print_symbols(state *s)
{
    ...
    {
        Elf32_Shdr *curr = &first_section[i];
        char *curr_name = shstrtab_start + first_section[i].sh_name;
        if (strcmp(curr_name, ".symtab") == 0)
        {
            symtab_sctn = &first_section[i];
            sym_amount = (symtab_sctn->sh_size) / symtab_sctn->sh_entsize;
        }
    ...
    Elf32_Shdr *sections = (Elf32_Shdr *)((int)s->hdr + s->hdr->e_shoff); //section header pointer
    ...
    for (int i = 0; i < sym_amount; i++)
    {
        ...
        else
        {
            printf("[%2d]  %-11x%-16d%-24s%-20s\n",
                   ...
                   shstrtab_start + first_section[curr_sym->st_shndx].sh_name, //curr_sym->st_shndx gives us the sectin number of the current symbol and first_section in that location gives us the elevant secation
                   strtble_start + curr_sym->st_name);
        }
    }
}
